...
def get_updates(offset=None):
    response = requests.get(BASE_URL + "getUpdates", params={"timeout": 100, "offset": offset})
...
def process_update(update):
    ...
    if text.startswith("/start"):
        parts = text.split()
        if len(parts) < 2:
            send_telegram_message("Будь ласка, введіть команду у форматі /start email", chat_id)
            return
